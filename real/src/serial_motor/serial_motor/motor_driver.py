...
class MotorDriver(Node):
    ...
    def send_feedback_motor_command(
        ...
        """Send PWM command instead of feedback control."""
        # Clamp values to PWM range [-255, 255]
        pwm1 = max(-100, min(100, int(mot_1_ct_per_loop)))
        pwm2 = max(-100, min(100, int(mot_2_ct_per_loop)))
        self.send_command(f"o {pwm1} {pwm2}")
    ...
    def send_command(self, cmd_string: str) -> Optional[str]:
        ...
        with self.mutex:
            try:
                ...
                #self._logger.warning(f"JUST SENT A COMMAND: {cmd_string}")
                if self.debug_serial_cmds:
